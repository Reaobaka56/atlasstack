...
class TestAnalysisHardening:
    ...
    def test_validate_invalid_domain_fails(self):
        ...
        assert "Invalid repository URL format" in exc.value.detail
    ...
    def test_validate_ssrf_fails(self):
        ...
        with pytest.raises(HTTPException) as exc:
            validate_repo_url(url)
        assert exc.value.status_code == 400
        assert "SSRF protected" in exc.value.detail or "Invalid repository URL format" in exc.value.detail
